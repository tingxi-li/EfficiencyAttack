...
class Pipeline(BasePipeline):
    def __init__(self, config, device=None):
        ...
        self.output_log = bool(config.get("output_log", True))
    ...
    def load_dataset(self):
        if self.dataset_name == "coco":
            ...
            if self.config.get("test_size") is not None:
        ...
        for _, example in tqdm(enumerate(dataset), total=len(dataset)):
            ...
            for step_idx in range(self.raja_steps):
                ...
                stage1_hinge = self._hinge_penalty(
                    probs_stage1, self.target_labels_stage1, self.conf_threshold_1, self.raja_hinge_weight_stage1
                )
                ...
                if self.raja_log_grad_decomp:
                    ...
                    if stage1_total.requires_grad:
                        grad_stage1 = torch.autograd.grad(
                            stage1_total,
                            x_adv,
                            retain_graph=retain_for_stage2,
                            allow_unused=True,
                        )[0]
                    ...
                    grad = torch.autograd.grad(total_loss, x_adv, retain_graph=False, allow_unused=True)[0]
                ...
                false_pos_stage1 = self._count_false_pos(probs_stage1, self.target_labels_stage1, self.conf_threshold_1)
                if self.raja_log_grad_decomp:
                    ...
                    log_entry.update(
                        {
                            ...
                            "roi_energy_ratio": roi_energy_ratio if roi_energy_ratio is not None else None,
                        }
                    )
    ...
    def _forward_stage2(self, x_adv, boxes_norm):
        ...
        for _ in range(eot_runs):
            ...
            patches = roi_align(
                x_adv,
                rois,
                output_size=self.raja_roi_size,
                aligned=True,
            )
            ...
            for start in range(0, patches.shape[0], self.model2_batch_size):
                ...
                chunk_hinge = self._hinge_penalty(
                    probs_stage2,
                    self.target_labels_stage2,
                    self.conf_threshold_2,
                    self.raja_hinge_weight_stage2,
                )
    ...
    def _jitter_boxes(self, boxes_norm):
        ...
        if self.raja_jitter_center > 0:
            noise = torch.randn_like(boxes[..., 0:2]) * self.raja_jitter_center
            boxes[..., 0:2] = boxes[..., 0:2] + noise

        if self.raja_jitter_size > 0:
            scale_noise = torch.randn_like(boxes[..., 2:4]) * self.raja_jitter_size
            boxes[..., 2:4] = boxes[..., 2:4] * (1.0 + scale_noise)
    ...
    def _extract_scores(self, probs, target_labels):
        ...
        selected = probs.index_select(dim=-1, index=idx)
    ...
    def _hinge_penalty(self, probs, target_labels, threshold, weight):
        ...
        total = scores_flat.numel()
        if total == 0:
            return probs.new_zeros(())
        topk_raw = int(self.raja_hinge_topk)
        if topk_raw <= 0:
            return probs.new_zeros(())
        topk = min(total, topk_raw)
        top_scores, _ = scores_flat.topk(topk)
        hinge = torch.clamp(top_scores - threshold, min=0.0)
    ...
    def _grad_norm(self, grad):
        if grad is None:
            return 0.0
        flat = grad.view(grad.shape[0], -1)
        return float(flat.norm(p=2, dim=1).mean().item())
    ...
    def _roi_energy_ratio(self, grad, boxes_xyxy, height, width):
        ...
        if total_energy.item() <= 0:
            return 0.0
        roi_energy = (grad_sq * mask).sum()
        return float((roi_energy / total_energy).item())
