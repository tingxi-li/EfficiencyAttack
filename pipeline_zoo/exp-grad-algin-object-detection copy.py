...
def run_attack(
    ...
    for step in range(steps):
        ...
        # gradient norms
        n_cls  = g_cls.norm() + 1e-8
        n_area = g_area.norm() + 1e-8
        n_norm = g_norm.norm() + 1e-8
        ...
        # normalized
        wsum = w_cls + w_area + w_norm
        w_cls, w_area, w_norm = w_cls/wsum, w_area/wsum, w_norm/wsum

        grad = w_cls * g_cls + w_norm * g_norm - w_area * g_area
...
def main():
    ...
    for sample in tqdm(subset):
        ...
        filename = f"{sample['image_id']}_steps_grad_align.png"
